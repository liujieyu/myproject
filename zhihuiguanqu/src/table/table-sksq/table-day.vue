    ...
    <div>
        <Content :style="{padding: '24px', background: '#fff'}">
                    ...
                    <Row style="font-size: 16px;">
                        ...
                        <Col style='font-size: 14px;text-align: center;'>共{{Tables.total}}个站点、涨水站数、降水站数、最大涨幅站点、涨幅、最大跌幅站点、跌幅<br>
                        平均水位：m，平均入库流量：m³/s，平均出库流量m³/s，日蓄水量：10⁶m³，汛限水位4-6月：m，汛限水位7-9月：m，日最高水位：m，日最低水位：m</Col>
                    </Row>
                    <el-table
                        :data="data"
                        border
              ...
              <el-dialog title="综合报表" :visible.sync="dialog_detail_showing" width="1200px" append-to-body>
                    ...
                    <Row>
                      ...
                      站点总数(个):{{Tables.total}}、涨水站数（个）：XX、           降水站数（个）：XX <br> 最大涨幅站点：A、            涨幅（m）：XX.XX、         最大跌幅站点：B、            跌幅（m）：XX.XX
                      </Col>
                      <Col style="text-align: center">平均水位：m，平均入库流量：m³/s，平均出库流量m³/s，日蓄水量：10⁶m³</Col>
    ...
    export default {
        ...
        methods:{
            ...
            Reload(){
                ...
                this.axios.get('/guanqu/table/ST_RSVR_D?_page_size='+this.list_input.pagesize+'&_page='+this.list_input.current+this.searchs,{params:{TM:this.form.date,ADDVCD:this.form.xzqh,Canal_Code:this.form.qd,CTYPE:this.form.zdlx,_orderby:this.form.field}}).then((res)=>{
                    console.log(res);
                    this.loading = false;
                    if(res.data.total == 0){
                ...
                this.axios.get('/guanqu/table/ST_RSVR_D?_page_size='+this.list_input.pagesize+'&_page='+this.list_input.current+this.searchs,{params:{TM:this.form.date,ADDVCD:this.form.xzqh,Canal_Code:this.form.qd,CTYPE:this.form.zdlx,_orderby:this.form.field}}).then((res)=>{
                    ...
                    for (var i = 0;i<this.Tables.data.length ; i++) {
                        this.Tables.data[i].DA_UPZ = this.Z_Filter(this.Tables.data[i].DA_UPZ);
                        this.Tables.data[i].DA_DWZ = this.Z_Filter(this.Tables.data[i].DA_DWZ);
