    ...
    <div>
        <Content class="searchcon" >
                    <Row type="flex" :gutter="16" class="rowtocol">
                        ...
                        <Col> 
                            <Button type="primary" @click="optable" >综合报表</Button>
                        </Col>
                        <Col> 
                        <Button type="primary" @click="err">导出</Button>
                        </Col>
                    </Row>
                    <Divider />
                    <Row style="font-size: 16px;">
                        ...
                        <Col style='font-size: 14px;text-align: center;'>共{{Tables.total}}个站点&nbsp;
                      平均水位：m，平均流量：m³/s，月累计水量：m³</Col>
                    </Row>
                    <el-table
                        :data="data"
                        border
                ...
                <el-dialog title="综合报表" :visible.sync="dialog_detail_showing" width="1200px" append-to-body>
                    ...
                    <Row>
                      ...
                      <Col>平均水位：m，平均流量：m³/s，月累计水量：m³</Col>
    ...
    export default {
        ...
        methods:{
            ...
            Reload(){
                ...
                this.axios.get('/guanqu/table/ST_MCanal_C?'+this.searchs,{params:{YE:this.form.YE,MON:this.form.MON,ADDVCD:this.form.xzqh,Canal_Code:this.form.qd,STYPE:this.form.zdlx}}).then((res)=>{
                    console.log(res);
                    this.loading = false;
                    // if(res.data.total == 0){
                    //   this.axios.get('/guanqu/table/ST_Canal_R1?_page=1&_page_size=99999').then((res)=>{
                    ...
                    //     console.log('data',this.data,'Tdata',this.Tables.data);
                    //   });
                    // }else{
                      this.data = res.data.rows;
                      ...
                      for (var i = 0;i<this.data.length ; i++) {
                          // this.data[i].MA_Q = '0'+this.data[i].MA_Q;
                          this.data[i].MA_Z = this.Z_Filter(this.data[i].MA_Z);
                ...
                this.axios.get('/guanqu/table/ST_MCanal_C?'+this.searchs,{params:{YE:this.form.YE,MON:this.form.MON,ADDVCD:this.form.xzqh,Canal_Code:this.form.qd,STYPE:this.form.zdlx}}).then((res)=>{
                    ...
                    for (var i = 0;i<this.data.length ; i++) {
                        this.Tables.data[i].MA_Z = this.Z_Filter(this.Tables.data[i].MA_Z);
                ...
                // this.axios.get('/guanqu/table/ST_MCanal_C').then((res)=>{
                // });
                //行政区划
                ...
                // this.axios.get('/guanqu/admin/ST_Canal_R').then((res)=>{
