	...
	<div>
		<Content :style="{padding: '24px', background: '#fff'}">
                    <Row type="flex" :gutter="16" justify="end">
                        ...
                        <Col>
                        <!-- 站名模糊搜索 -->
                            <Input search enter-button suffix="ios-search" placeholder="请输入沉沙池名称" style="width: auto;margin-right: 10px;" @on-search="search" v-model="form.searchmsg" />
                        </Col>
                    </Row>
...
<script type="text/javascript">
	...
	export default {
        ...
        methods:{
            ...
            Reload(){
                this.loading = true;
                this.axios.get('/guanqu/admin/WRP_IrrBTSediBasin?_page_size='+this.list_input.pagesize+'&_page='+this.list_input.current).then((res)=>{
                    console.log(res);
                    this.loading = false;
                    this.data = res.data.rows;
                    this.list_input.total = res.data.total;
                    for (var i = 0;i<this.data.length ; i++) {
                      this.data[i].D_FL = this.Z_Filter(this.data[i].D_FL,3);
                      this.data[i].ADD_FL = this.Z_Filter(this.data[i].ADD_FL,3);
