  ...
  methods: {
    ...
    showTips(item) {
      ...
      var div = `<div>
        <div class="tip_row">
              ...
              <div class="divider"></div>
              <div class="tip_row">
                <span class="tip_col_5">预警闸前水位：</span>
                <span class="tip_col_7" id="${item.STCD}_YJZQ_Z">
                  
                </span>
                <span class="tip_col_5" >预警闸后水位：</span>
    ...
    search() {
      ...
      if (this.form.model_site) {
        body["ST_WAS_R1.CTYPE"] = this.form.model_site;
      }

      this.tableLoading(); // 表格加载中
      this.search_FeatrueLayer(5, body, this.featrue, newFeatureLayerOBJ => {
        this.baseBox_init();
