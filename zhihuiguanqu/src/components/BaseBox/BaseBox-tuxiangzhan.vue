  ...
  methods: {
    ...
    showTips(item) {
      ...
      setTimeout(() => {
        ...
        this.map.infoWindow.resize(180, 650); // 提示框大小
        this.map.infoWindow.setTitle(item.STNM); // 提示题目
        this.map.infoWindow.setContent(element); // 提示内容
        var zoompoint = {
    ...
    search() {
      ...
      if (this.form.model_adress && this.form.model_adress.length != 0) {
        body["ST_STBPRP_B.ADDVCD"] = `like,${this.$App.ADDVCD_Array_Filter(this.form.model_adress)}`;
      }
      // 如果渠道选择框有内容，添加渠道过滤字段
      if (this.form.model_qudao && this.form.model_qudao.length != 0) {
        body["WRP_IrrBTCanalSystem.Canal_Code"] = `like,${this.$App.Canal_Code_Array_Filter(this.form.model_qudao)}`;;
      }

      this.tableLoading(); // 表格加载中
      this.search_FeatrueLayer(7, body, this.featrue, newFeatureLayerOBJ => {
        this.baseBox_init();
